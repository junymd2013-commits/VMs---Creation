...
with col1:
    st.markdown("### 直線①（青）")
    a1 = st.slider("傾き a₁", -5.0, 5.0, float(st.session_state.a1), 0.1)
    b1 = st.slider("切片 b₁", -5.0, 5.0, float(st.session_state.b1), 0.1)

with col2:
    st.markdown("### 直線②（赤）")
    a2 = st.slider("傾き a₂", -5.0, 5.0, float(st.session_state.a2), 0.1)
    b2 = st.slider("切片 b₂", -5.0, 5.0, float(st.session_state.b2), 0.1)
